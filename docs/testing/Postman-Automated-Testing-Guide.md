...
## แนวทางการเขียนคอมเมนต์ (ภาษาไทย/อังกฤษ)

สามารถใช้คอมเมนต์ภาษาไทยได้ และแนะนำให้ใส่ "เหตุผล" กำกับคอมเมนต์สำคัญ โดยเฉพาะจุดที่คนอ่านอาจสงสัยว่า "ทำไมต้องทำแบบนี้"

ตัวอย่างรูปแบบที่แนะนำ:

```javascript
// TH: บันทึก token เพื่อใช้กับ request ถัดไป
// EN: Save token for chained requests
// Why: ลดการ login ซ้ำทุก endpoint และทำให้ flow ทดสอบต่อเนื่องได้
pm.environment.set("auth_token", json.data.accessToken);
```

หลักการสั้นๆ:
- คอมเมนต์เพื่ออธิบาย "เหตุผล" ไม่ใช่แค่อธิบายสิ่งที่โค้ดทำอยู่แล้ว
- ถ้าทีมเป็นไทยล้วน ใช้ไทยล้วนได้
- ถ้ามีทีมผสมหรือส่งต่อข้ามทีม ให้ใช้ไทย + อังกฤษในจุดสำคัญ
